...
const ForgotPassword = () => {
  ...
  const handleSendOTP = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("send-otp", { body: { email, action: "send" } });
      if (error) throw error;
      toast({ title: "OTP Sent", description: "If your email exists in our system, you'll receive a code shortly." });
      setStep("otp");
    } catch (error: any) {
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("send-otp", { body: { email, otp, action: "verify" } });
      if (error) throw error;
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const { data, error } = await supabase.functions.invoke("send-otp", { body: { email, resetToken, newPassword, action: "reset" } });
      if (error) throw error;
      if (data.error) throw new Error(data.error);
    ...
    <div className="min-h-screen flex">
        ...
        <div className="w-full max-w-md">
          <Link to="/" className="flex items-center gap-2 mb-8">
            <div className="relative">
              ...
              <Sparkles className="absolute -top-1 -right-1 w-4 h-4 text-true-azure-500" />
            </div>
              ...
              <form onSubmit={handleVerifyOTP} className="space-y-6">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="otp" type="text" placeholder="123456" value={otp} onChange={(e) => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} className="pl-10 h-12 text-center text-2xl tracking-widest font-mono" maxLength={6} required />
                  </div>
                </div>
                <Button type="submit" variant="hero" className="w-full" size="lg" disabled={isLoading}>
      ...
      <div className="hidden lg:flex flex-1 items-center justify-center relative overflow-hidden gradient-hero">
        ...
        <div className="relative z-10 text-center p-12">
          <div className="w-32 h-32 rounded-2xl gradient-purple shadow-glow mx-auto mb-8 flex items-center justify-center animate-float">
        ...
        <div className="absolute top-20 left-20 w-40 h-40 bg-ultrasonic-blue-400/30 rounded-full blur-3xl animate-float" />
        <div className="absolute bottom-20 right-20 w-60 h-60 bg-raspberry-plum-400/30 rounded-full blur-3xl animate-float-delayed" />
        <div className="absolute top-1/3 right-10 w-32 h-32 bg-blue-energy-400/30 rounded-full blur-3xl" />
